    ...
    <div class="space-y-6">
        ...
        <div class="bg-bg-card border border-border rounded-card p-4 shadow-sm">
            <div class="flex items-center space-x-3">
                <div class="flex-1 relative">
                    ...
                    <input v-model="adminStore.userSearch" @input="handleSearch" type="text"
                ...
                <button v-if="adminStore.userSearch" @click="adminStore.clearUserSearch"
        ...
        <div v-else class="bg-bg-card shadow-sm rounded-card border border-border">
            ...
            <ul v-else class="divide-y divide-border">
                <li v-for="user in adminStore.allUsers" :key="user.id"
                    ...
                    <div class="flex items-center justify-between">
                            ...
                            <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div class="text-center">
                                    <div class="text-2xl font-bold text-primary">
                                        {{ user.total_tasks > 0 ? ((user.completed_tasks || 0) / user.total_tasks *
                                            100).toFixed(1) : 0 }}%
                                    </div>
                        ...
                        <div class="flex items-center space-x-2">
                            <button @click="viewUserStats(user.id)"
        ...
        <!-- User Stats Modal -->
        <div v-if="showUserStatsModal" class="fixed inset-0 bg-bg-overlay overflow-y-auto h-full w-full z-50"
            @click="closeUserStatsModal">
            <div class="relative top-20 mx-auto p-5 border border-border w-96 shadow-lg rounded-card bg-bg-card"
                @click.stop>
                <div class="mt-3">
                    ...
                    <div v-if="selectedUserStats" class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="text-center p-3 bg-bg-secondary rounded-card">
                                <div class="text-2xl font-bold text-success">{{ selectedUserStats.completion_rate }}%
                                </div>
                                <div class="text-sm text-text-muted">Completion Rate</div>
                            </div>
                        </div>
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-sm font-medium text-text">{{ selectedUserStats.completed_tasks
                                }}</span>
                            </div>
                            <div class="flex justify-between">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-sm font-medium text-text">{{ selectedUserStats.medium_priority
                                }}</span>
                            </div>
                            <div class="flex justify-between">
                    ...
                    <div class="mt-6 flex justify-end">
                        <button @click="closeUserStatsModal"
                            class="px-4 py-2 text-sm font-medium text-text bg-bg-secondary border border-border rounded-card hover:bg-bg-tertiary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-colors duration-200">
                            Close
                        </button>
                    </div>
